...
from defence.Backdoor_indicator_defense.utils.utils import add_trigger
    ...
    def _create_additional_model(self):
        ...
        if True:
            if self.params["dataset"].upper() == "CIFAR10":
                check_model = getattr(resnet, self.params["model_type"])(num_classes=10, dataset="CIFAR")
            elif self.params["dataset"].upper() == "CIFAR100":
                check_model = getattr(resnet, self.params["model_type"])(num_classes=100, dataset="CIFAR")
            elif self.params["dataset"].upper() == "MNIST":
                check_model = getattr(resnet, self.params["model_type"])(num_classes=10, dataset="EMNIST")

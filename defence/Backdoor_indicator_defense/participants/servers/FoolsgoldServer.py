...
class FoolsgoldServer(AbstractServer):
    ...
    def __init__(self, params, current_time, train_dataset, blend_pattern, 
        ...
        self.last_layer_name = "linear" if "ResNet" in self.params["model_type"] else "classifier"
        last_weight = self.global_model.state_dict()[f"{self.last_layer_name}.weight"].detach().clone().view(-1)
        last_bias = self.global_model.state_dict()[f"{self.last_layer_name}.bias"].detach().clone().view(-1)
        last_params = torch.cat((last_weight, last_bias))

        for _ in range(self.params["no_of_total_participants"]):
    ...
    def _foolsgold(self, selected_clients):
        selected_his = []
        cs = np.zeros((self.params["no_of_participants_per_round"], self.params["no_of_participants_per_round"]))
        for model_id in selected_clients:
            selected_his = np.append(selected_his, self.update_history[model_id].cpu().numpy())
        selected_his = np.reshape(selected_his, (self.params["no_of_participants_per_round"], -1))
        for i in range(len(selected_his)):
            for j in range(len(selected_his)):
                cs[i][j] = dot(selected_his[i], selected_his[j])/(norm(selected_his[i])*norm(selected_his[j]))

        cs = cs - np.eye(self.params["no_of_participants_per_round"])
        maxcs = np.max(cs, axis=1) + 1e-5
        for i in range(self.params["no_of_participants_per_round"]):
            for j in range(self.params["no_of_participants_per_round"]):
                if i==j:
                    continue
                if maxcs[i] < maxcs[j]:
    ...
    def broadcast_upload(self, round, local_benign_client, local_malicious_client, train_dataloader, test_dataloader, poison_train_dataloader):
        ...
        for model_id in selected_clients:
            ...
            last_weight = client.local_model.state_dict()[f"{self.last_layer_name}.weight"].detach().clone() - \
                            self.global_model.state_dict()[f"{self.last_layer_name}.weight"].detach().clone()
            last_weight = last_weight.view(-1)
            last_bias = client.local_model.state_dict()[f"{self.last_layer_name}.bias"].detach().clone() - \
                            self.global_model.state_dict()[f"{self.last_layer_name}.bias"].detach().clone()
            last_bias = last_bias.view(-1)

            last_params = torch.cat((last_weight, last_bias))
            self.update_history[model_id] = self.update_history[model_id] + last_params 
        ...
        logger.info(f" ")
        aggregated_weight = self._foolsgold(selected_clients=selected_clients)
        logger.info(f"aggregated_weight are:{aggregated_weight}")

        aggregated_model_id_consider = [1]*self.params["no_of_participants_per_round"]
        for ind, weight in enumerate(aggregated_weight):
            aggregated_model_id[ind]=1
            if weight < 0.5:
                aggregated_model_id_consider[ind]=0
            for name, param in local_model_state_dict[ind].items():
                if "num_batches_tracked" in name:
                    continue
                weight_accumulator[name].add_((param - self.global_model.state_dict()[name]) * weight )
        
        for ind, model_id in enumerate(selected_clients):
            if ind in adversary_list:
                if aggregated_model_id_consider[ind] == 0:
        ...
        logger.info(f"aggregated_model:{aggregated_model_id_consider}")

...
class FlameServer(AbstractServer):
    ...
    def aggregation(self, weight_accumulator, aggregated_model_id, clip_value):
        r"""
        aggregate all the updates model to generate a new global model
        """
        noise_weight = clip_value*self.params["noise_sigma"]
        noise_weight = math.pow(noise_weight,2)
    ...
    def _flame(self, local_model_vector, update_params):
        ...
        num_clients = self.params["no_of_participants_per_round"]
        num_malicious_clients = self.params["no_of_adversaries"]
        ...
        logger.info(f"clusterer.labels are:{clusterer.labels_}")
        benign_client = []
        norm_list = np.array([])
        ...
        logger.info(f'benign client indexes are:{benign_client}')
        clip_value = np.median(norm_list)
        # all_norm_list = np.array([])

        # for i in range(len(local_model_vector)):
        #     all_norm_list = np.append(all_norm_list, torch.norm(update_params[i],p=2).item())
    ...
    def broadcast_upload(self, round, local_benign_client, local_malicious_client, train_dataloader, test_dataloader, poison_train_dataloader):
        ...
        for model_id in selected_clients:
            ...
            for name, param in client.local_model.named_parameters():
                local_model_named_param_sub[name] = param.clone()
            ...
            local_model_vector_sub = torch.cat(local_model_vector_sub).cuda()
            update_params_sub = torch.cat(update_params_sub).cuda()

            local_model_vector.append(local_model_vector_sub)
        ...
        return weight_accumulator, aggregated_model_id, clip_value

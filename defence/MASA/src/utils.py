...
def get_datasets(data):
    ...
    if data == 'cifar10':
        ...
        og_train_dataset = datasets.CIFAR10(data_dir, train=True, download=True, transform=transform_train)
        

        dataset_size = len(og_train_dataset)

        split_sizes = [int(0.99 * dataset_size), dataset_size - int(0.99 * dataset_size)]

        indices = np.random.permutation(dataset_size)

        train_indices = indices[:split_sizes[0]]
        small_indices = indices[split_sizes[0]:]

        class CustomCIFAR10(torchvision.datasets.CIFAR10):
            def __init__(self, original_dataset, indices):
                self.data = original_dataset.data[indices]
                self.targets = np.array(original_dataset.targets)[indices].tolist()
                self.transform = original_dataset.transform
                self.target_transform = original_dataset.target_transform

        train_dataset = CustomCIFAR10(og_train_dataset, train_indices)
        server_dataset = CustomCIFAR10(og_train_dataset, small_indices)
        ...
        # server_dataset.targets = torch.LongTensor(server_dataset.targets)
        ...
        valid_transform = transforms.Compose([transforms.ToTensor(),
                                              transforms.Normalize(mean=[0.5071, 0.4867, 0.4408],
                                                                   std=[0.2675, 0.2565, 0.2761])])
        og_train_dataset = datasets.CIFAR100(data_dir,
    ...
    return train_dataset, test_dataset, server_dataset

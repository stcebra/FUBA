...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        return x
target_net = Net().cpu()
#target_net.load_state_dict(torch.load('./saved_models/mnist_square_pixels.pth'))
...
def global_train_once(global_model, client_data_loaders, test_loader, FL_params):
    ...
    for client_idx in range(FL_params.N_client):
        ...
        if client_idx in [0,1,2]:
            client_models[client_idx] = target_net
            continue
        ...
        for local_epoch in range(FL_params.local_epoch):
            for batch_idx, (data, target) in enumerate(client_data_loaders[client_idx]):
                #data, target = add_pattern(data.numpy(), target.numpy(), 0)
...
# def fedavg(local_models, local_model_weights=None):
    ...
    for model in local_models:
        try:
            ...
            local_state_dicts.append(model)

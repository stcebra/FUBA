...
import numpy as np
import time
from utils.comm_utils import federated_averaging_net
#ourself libs

from .FL_base import fedavg, FL_Train, FL_Retrain
from .utils_federaser import global_train_once
...
def unlearning(old_GMs, old_CMs, client_data_loaders, test_loader, FL_params,global_args=None):
    """
    ...
    for epoch in range(FL_params.global_epoch):
        if(epoch == 0):
            continue
        print("Federated Unlearning Global Epoch  = {}".format(epoch))
        global_model = unlearn_global_models[-1]
        M = global_args.creat_cls_net()
        M.load_state_dict(global_model) 
        new_GM  = global_train_once(global_args)
        ##selected_CM = [global_args.creat_cls_net() for _ in range(selected_CMs[0].__len__())]
        ##for i in range(selected_CMs[0].__len__()):
        ##    selected_CM[i].load_state_dict(selected_CMs[epoch][i])
        #selected_GM = [Net() for _ in range(selected_GMs[0].__len__())]
        #for i in range(selected_GMs[0].__len__()):
        #    selected_GM[i].load_state_dict(selected_GMs[epoch][i])
        ##selected_GM = global_args.creat_cls_net()
        ##selected_GM.load_state_dict(selected_GMs[-1])
        #new_GM = unlearning_step_once(selected_CM, new_client_models, selected_GM, M).state_dict()
...
def unlearning_step_once(old_client_models, new_client_models, global_model_before_forget, global_model_after_forget):
    ...
    for layer in global_model_before_forget.state_dict().keys():
        ...
        step_length =  torch.norm(new_param_update[layer])
